<x-layouts.app :title="$title ?? 'Roles'">
                ...
                <table class="w-full table-auto text-left text-sm">
                    <thead>
                        <tr class="bg-zinc-300 dark:bg-zinc-700">
                            ...
                            <th class="p-3 text-zinc-900 dark:text-white font-semibold">Descripción</th>
                    ...
                    <tbody>
                        @foreach ($roles as $rol)
                            <tr class="{{ $tableRowClass }}">
                                <td class="p-3 text-zinc-900 dark:text-zinc-100 font-medium">{{ $rol->nombre }}</td>
                                <td class="p-3 text-zinc-700 dark:text-zinc-300">{{ $rol->description ?? '' }}</td>
                                <td class="p-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('roles.destroy', $rol) }}" method="POST"
                                            style="display:inline;" onsubmit="return confirm('¿Eliminar rol?');">
                                            @csrf @method('DELETE')
                                            <button type="submit"
