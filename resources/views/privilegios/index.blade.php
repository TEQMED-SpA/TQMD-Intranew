<x-layouts.app :title="$title ?? 'Privilegios'">
    <div class="min-h-screen px-4 py-8 bg-zinc-50 dark:bg-zinc-800 transition-all">
        <div class="max-w-6xl mx-auto">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-zinc-800 dark:text-white">Privilegios</h1>
                ...
                <table class="w-full table-auto text-left text-sm">
                    <thead>
                        <tr class="bg-white dark:bg-zinc-700">
                            ...
                            <th class="p-3 text-zinc-700 dark:text-white font-semibold">Tipo</th>
                    ...
                    <tbody>
                        @foreach ($privilegios as $privilegio)
                            <tr class="{{ $tableRowClass }}">
                                <td class="p-3 text-zinc-900 dark:text-zinc-100 font-medium">{{ $privilegio->nombre }}
                                </td>
                                <td class="p-3 text-zinc-900 dark:text-zinc-300">{{ $privilegio->descripcion }}</td>
                                <td class="p-3 text-zinc-900 dark:text-zinc-300">{{ $privilegio->tipo }}</td>
                                <td class="p-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('privilegios.show', $privilegio) }}"
                                        ...
                                        <form action="{{ route('privilegios.destroy', $privilegio) }}" method="POST"
                                            style="display:inline;" onsubmit="return confirm('¿Eliminar privilegio?');">
                                            @csrf @method('DELETE')
                                            <button type="submit"
